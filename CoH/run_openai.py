...
from openai_utils import predict_chatgpt, predict_chatgpt_cot_step1 
from utils import (
    GenerativeMetric,
    adjust_top_k,
    get_args,
...
if __name__ == "__main__":
    ...
    with torch.no_grad(), open(filename, "w", encoding="utf-8") as writer, tqdm(test_data) as pbar:
        for i, x in enumerate(pbar):
            ...
            step1_prompt = prepare_input(
                x, search_space, background_space, ent_dict, rel_dict, entity_2_des, 
                args, return_prompt=True, is_cot_step1=True  
            )
           
            important_entities = predict_chatgpt_cot_step1(step1_prompt, args)
            ...
            if args.model == "chatgpt":
                predictions = predict_chatgpt(model_input, args, important_entities=important_entities)
            else:
