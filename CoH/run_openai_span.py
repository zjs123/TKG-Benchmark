...
from openai_utils import predict_chatgpt_span, predict_chatgpt_span_cot_step1  # 新增Step1函数
from utils import (
    MAEMetric,
    get_args,
    get_filename,
    ...
    with torch.no_grad(), open(filename, "w", encoding="utf-8") as writer, tqdm(test_timestamps[1:]) as pbar:
        for i, timestamp in enumerate(pbar):
            ...
            for test_fact in tmp_test_samples:
                ...
                step1_prompt = prepare_input_span(
                    test_fact, timestamp, background_space, ent_dict, rel_dict, entity_2_des, 
                    args, return_prompt=True, is_cot_step1=True
                )
                important_entities = predict_chatgpt_span_cot_step1(step1_prompt, args)
                
                model_input = prepare_input_span(
                    test_fact, timestamp, background_space, ent_dict, rel_dict, entity_2_des, 
                    args, return_prompt=True
                )
                predictions = predict_chatgpt_span(model_input, args, important_entities=important_entities)
